...
install_apt_tools() {
    ...
    for tool in "${APT_TOOLS[@]}"; do
        if dpkg -s "$tool" &>/dev/null; then
            ...
            apt-get install -y -qq "$tool" 2>>"$LOG_FILE" && pass "$tool installed" || warn "$tool failed — check $LOG_FILE"
        fi
    done
}
...
go_install() {
    ...
    if GOPATH="$HOME/go" go install "$pkg" 2>>"$LOG_FILE"; then
        # Copy to /usr/local/bin so it's in PATH for all users
...
install_trufflehog() {
    ...
    curl -sSfL https://raw.githubusercontent.com/trufflesecurity/trufflehog/main/scripts/install.sh \
        | sh -s -- -b /usr/local/bin 2>>"$LOG_FILE" && pass "trufflehog installed" || warn "trufflehog failed — check $LOG_FILE"
}
...
install_gitleaks() {
    ...
    curl -sSL "$GL_URL" -o /tmp/gitleaks.tar.gz 2>>"$LOG_FILE"
...
install_cloud_tools() {
    ...
    # cloud_enum
    if command -v cloud_enum &>/dev/null || [[ -f /opt/cloud_enum/cloud_enum.py ]]; then
        pass "cloud_enum already installed"
    else
        info "Installing cloud_enum..."
        mkdir -p /opt/recon-tools
        git clone -q https://github.com/initstring/cloud_enum.git /opt/recon-tools/cloud_enum 2>>"$LOG_FILE"
        pip3 install -q -r /opt/recon-tools/cloud_enum/requirements.txt 2>>"$LOG_FILE"
        ln -sf /opt/recon-tools/cloud_enum/cloud_enum.py /usr/local/bin/cloud_enum
    ...
    if command -v s3scanner &>/dev/null; then
        ...
        pip3 install -q s3scanner 2>>"$LOG_FILE" && pass "s3scanner installed" || warn "s3scanner failed — check $LOG_FILE"
    fi
}
...
update_nuclei_templates() {
    ...
    if command -v nuclei &>/dev/null; then
        ...
        nuclei -update-templates -silent 2>>"$LOG_FILE" && pass "Nuclei templates updated" || warn "Template update failed"
    else
...
verify_tools() {
    ...
    if $ALL_OK; then
        ...
        warn "Some tools failed — review $LOG_FILE"
        warn "Re-run the script or install failed tools manually"
    fi
}
...
echo ""
pass "Done. Log saved to $LOG_FILE"
echo ""
